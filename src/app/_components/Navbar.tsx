...
import { homeLoadedAtom, isNavBlurredAtom } from "~/lib/state"
import {
  motion,
  useMotionValue,
...
const Navbar: React.FC<NavbarProps> = ({ className = "" }) => {
  ...
  const isNavBlurred = useAtomValue(isNavBlurredAtom)

  const hideNav = () => {
    const navbar = navRef.current
    if (navbar) {
      navbar.classList.add("-translate-y-[130%]")
    }
  }
  ...
  const showNav = () => {
    const navbar = navRef.current
    if (navbar) {
      navbar.classList.remove("-translate-y-[130%]")
    }
  }
  ...
  useEffect(() => {
    if (homeLoaded) {
      void animate(animationSequence, { delay: 0.5 })
    }
  }, [homeLoaded])
  ...
  return (
    <>
      <header
        ref={navRef}
        className={twMerge(
          "fixed left-0 right-0 top-0 z-40 flex flex-col items-center transition-transform duration-500",
          !isNavBlurred && "mix-blend-difference",
          className,
        )}
      >
        <div className="relative contents w-full">
          <div
            className={twMerge(
              "nav-blur absolute -bottom-14 left-0 right-0 top-0 -z-10",
              !isNavBlurred && "hidden",
            )}
          />
          <div ref={animationScope} className="z-10 contents w-full">
            <motion.nav
